					...
					<div class="title" id="conf">
						...
						<div class="row">
							...
							<div class="col-lg-7" class="lead">
								...
								<hr>
								<form:form id="f1" name="confirform" action="validar-pedido"
									method="POST" modelAttribute="pedido">
									<div class="col-xs-3">
										<p class="lead">Nombre:</p>
										<form:input id="nombre" type="text" path="solicitante"
											class="form-control" placeholder="Juan" />
									</div>
									<div class="col-xs-4">
										<p class="lead">Direccion:</p>
										<form:input id="direccion" type="text" path="direccion"
											class="form-control" placeholder="AV Corrientes 2740" />
										<span id="direerror"></span>
									</div>
									<div class="col-xs-5">
										<p class="lead">Telefono:</p>
										<form:input id="telefono" type="text" path="telefono"
							...
							<div class="box">
								<c:choose>
									<c:when test="${not empty pedidos}">
										...
										<div id="dspbtn"></div>
										<button id="tresp" type="button"
											class="btn btn-lg btn-primary btn-block"
											onclick="valida_envia()">Enviar Pedido</button>
				...
				<form class="form-horizontal" role="form" id="form-crear">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary" data-dismiss="modal"
							onClick="javascript: document.confirform.submit();">
							<span class="hidden-xs">Confirmar</span>
						</button>
						<button type="button" class="btn btn-warning" data-dismiss="modal"
							onclick="valida_noenvia()">
...
<script type="text/javascript"
	src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBCGMlKnUq66Cb1jaJwUHBqqUurzJPJ7s8"></script>
					...
					function() {
						...
						if (inputDireccion == '') {
							$("#direccion")
									.focus()
									.after(
											"<span class='error'>Ingrese su direccion</span>");
						}
						;

						if (inputDireccion != '' && inputDireccion != ' '
								&& inputDireccion != '  '
								&& inputDireccion != '   ') {

							var url = 'https://www.google.com/maps/embed/v1/place?key=AIzaSyA2mfgpfvZWDfry05R1F52Gj8V_zBGeYbo&q='
									+ inputDireccion;
							',Buenos+Aires'
											...
											function(response, status) {
												if (status === google.maps.DirectionsStatus.OK) {
													...
													var horas = Math
															.floor(duration / 3600);
													var minutos = Math
															.floor((duration - (horas * 3600)) / 60);
													var segundos = Math
															.round(duration
																	- (horas * 3600)
																	- (minutos * 60));

													function conversor_horas(
															duration) {
														var horas = Math
																.floor(duration / 3600);
														var minutos = Math
													...
													if (horas > 0) {
														...
														document
																.getElementById('direerror').innerHTML = '';
														vdireccion = true;
													} else {
														directionsDisplay
																.setDirections(response);
													...
													directionsDisplay
															.setDirections(response);
													document
															.getElementById('mapa').style.visibility = 'hidden';
													document
															.getElementById('ifra').style.height = '0';
													document
															.getElementById('direerror').style.padding = '6px 12px';
													document
															.getElementById('direerror').innerHTML = 'Direccion no valida';
													document
	...
	$("#telefono").blur(
			function() {
				...
				if (telefono == '' || telefono.length < 9) {
					$("#telefono").focus().after(
							"<span class='error'>Ingrese su telefono</span>");
					vtelefono = false;
				} else {
					vtelefono = true;
				}
	...
	function valida_envia() {
		...
		if (vnombre == true && vdireccion == true && vtelefono == true) {
			...
			$("#dspbtn")
					.focus()
					.after(
							'<button  id="trespue" type="button" class="btn btn-lg btn-primary btn-block" data-toggle="modal" data-target="#ModalCrear">Enviar Pedido</button>');
			$('#ModalCrear').modal('show');

		} else {
			$(".error").remove();
			$("#tresp").focus().after(
					"<span class='error'>Primero complete los campos</span>");
		}
	}
